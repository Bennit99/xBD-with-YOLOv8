    ...
    def load_image(self, i):
        ...
        if im is None:  # not cached in RAM
            if fn.exists():  # load npy
                ...
                f_pre = f.replace('post', 'pre')
                index = f_pre.rfind('Data\') + 4
                f_pre = f_pre[:index] + '\pre' + f_pre[index:]
                im_pre = cv2.imread(f_pre)
                if im_pre is None:
                    raise FileNotFoundError(f"pre Image Not Found {f_pre}")
                else: im = np.concatenate((im_pre, im), axis=2)
                '''---------------------------------added-by-Bennit--------------------------------------------------'''
                if im is None:
