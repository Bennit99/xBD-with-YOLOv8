...
class LoadImages:
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            ...
            '''---------------------------------added-by-Bennit--------------------------------------------------'''
            ''' load pre desater image in adition to post desater image and combine them '''
            path_pre = path.replace('post', 'pre')
            index = path_pre.rfind('Data\') + 4
            path_pre = path_pre[:index] + '\pre' + path_pre[index:]
            im_pre = cv2.imread(path_pre)
            if im_pre is None:
                raise FileNotFoundError(f"pre Image Not Found {path_pre}")
            else: im0 = np.concatenate((im_pre, im0), axis=2)
            '''---------------------------------added-by-Bennit--------------------------------------------------'''
            if im0 is None:
