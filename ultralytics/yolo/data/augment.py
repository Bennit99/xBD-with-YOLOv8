    ...
    def __call__(self, labels):
        ...
        if self.hgain or self.sgain or self.vgain:
            ...
            for i in range(img.shape[2]//3):
                img[:,:,i*3:i*3+3] = self.hsv(img[:,:,i*3:i*3+3], r)
        return labels
...
class LetterBox:
    ...
    def __call__(self, labels=None, image=None):
        ...
        if any([top, bottom, left, right]):
            if img.shape[2] == 6:
                img = np.concatenate((\
                    cv2.copyMakeBorder(img[:,:,0:3], top, bottom, left, right, cv2.BORDER_CONSTANT,value=(114, 114, 114)),
                    cv2.copyMakeBorder(img[:,:,3:6], top, bottom, left, right, cv2.BORDER_CONSTANT,value=(114, 114, 114))),
                    axis=2)
            else: img = cv2.copyMakeBorder(img, top, bottom, left, right, cv2.BORDER_CONSTANT,
